...
class Outputer_TTbar(Outputer):
    ...
    def fill_event(self, inTree, event, jet1, sel_mu, btag_jet):
        ...
        #PFCands = list(Collection(event, "PFCands")) #nanov12
        PFCands = list(Collection(event, "PFCand"))  #nanov15

        #PFCandsIdxs = list(Collection(event, "FatJetPFCands"))       #nanov12
        PFCandsIdxs = list(Collection(event, "FatJetPFCand")) #nanov15
        ...
        for i,conv in enumerate(range1):
            #idx = conv.pFCandsIdx 
            idx =conv.pfCandIdx #nanov15
            puppiw=1.0

            pdgId = PFCands[idx].pdgId
            absId = abs(pdgId)

            if absId in [211, 11, 13]:   # charged hadrons, electrons, muons
                charge = +1 if pdgId > 0 else -1
            else:
                charge = 0
            #if(i > j1_nPF): break
            if i >= self.n_pf_cands: break
            ...
            jet1_PFCands.append([cand.Px(), cand.Py(), cand.Pz(), cand.E(), puppiw, charge])
...
def NanoReader_TTbar(process_flag, inputFileNames=["in.root"], outputFileName="out.root", json = '', year = "2018", nEventsMax = -1, sampleType = "MC", 
    ...
    #triggers = ["HLT_Mu50"]
    triggers = ["HLT_Mu50","HLT_IsoMu24","HLT_CascadeMu100","HLT_HighPtTkMu100"]
...
# -------- Begin Loop over tree-------------------------------------
        ...
        for entry in range(entries):
            ...
            for i,jet in enumerate(AK8Jets):
                ...
                FatjetIdtight = False
                if abs(jet.eta) <= 2.6:  
                   FatjetIdtight = ((jet.neHEF < 0.99) and (jet.neEmEF < 0.9) and (jet.chMultiplicity+jet.neMultiplicity > 1) and (jet.chHEF > 0.01) and (jet.chMultiplicity > 0) )
                elif (abs(jet.eta) > 2.6 and abs(jet.eta) <= 2.7):
                   FatjetIdtight = (jet.neHEF < 0.90) and (jet.neEmEF < 0.99)
                elif (abs(jet.eta) > 2.7 and  abs(jet.eta) <= 3.0):
                   FatjetIdtight = (jet.neHEF < 0.99)
                elif (abs(jet.eta) > 3.0):
